...
func TestAdaptiveLimiter_OnResponseLowRemainingThrottlesBeforeZero(t *testing.T) {
	l := NewAdaptiveLimiter(8.0)
	l.OnResponse(rateLimitResponse("100", "50", "window-1"))
	startRate := l.Rate()

	l.OnResponse(rateLimitResponse("100", "9", "window-1"))
	lowRate := l.Rate()
	if lowRate >= startRate {
		t.Fatalf("Rate() after Remaining below 10%% = %v, want below %v", lowRate, startRate)
	}

	l.OnResponse(rateLimitResponse("100", "5", "window-1"))
	if got := l.Rate(); got >= lowRate {
		t.Fatalf("Rate() after progressively lower Remaining = %v, want below %v", got, lowRate)
	}
}

func TestAdaptiveLimiter_OnResponseMissingHeadersFallsBackTo429Only(t *testing.T) {
	l := NewAdaptiveLimiter(8.0)
	l.OnResponse(&http.Response{Header: http.Header{}})
	if got := l.Rate(); got != 8.0 {
		t.Fatalf("Rate() after missing X-RateLimit headers = %v, want unchanged 8", got)
	}

	l.OnRateLimit()
	if got := l.Rate(); got != 4.0 {
		t.Fatalf("Rate() after fallback OnRateLimit = %v, want 4", got)
	}
}

func TestAdaptiveLimiter_OnResponseWindowBoundaryRampsUp(t *testing.T) {
	l := NewAdaptiveLimiter(8.0)
	l.OnResponse(rateLimitResponse("100", "50", "window-1"))
	l.OnResponse(rateLimitResponse("100", "5", "window-1"))
	lowRate := l.Rate()

	l.OnResponse(rateLimitResponse("100", "99", "window-2"))
	if got := l.Rate(); got <= lowRate {
		t.Fatalf("Rate() after X-RateLimit-Window transition = %v, want above %v", got, lowRate)
	}
}

func TestAdaptiveLimiter_DefaultsToMemoryBackend(t *testing.T) {
	t.Setenv(ppRateLimitBackendEnv, "")
	t.Setenv(ppRateLimitPGURLEnv, "")

	l := NewAdaptiveLimiterForHost(8.0, "https://api.example.com/v1")
	if _, ok := l.backend.(*memoryRateLimitBackend); !ok {
		t.Fatalf("backend = %T, want *memoryRateLimitBackend", l.backend)
	}
	if l.key.Host != "api.example.com" {
		t.Fatalf("host key = %q, want api.example.com", l.key.Host)
	}
}

func TestAdaptiveLimiter_PostgresBackendFromEnv(t *testing.T) {
	t.Setenv(ppRateLimitBackendEnv, "postgres")
	t.Setenv(ppRateLimitPGURLEnv, "postgres://user:pass@localhost:5432/theclose?sslmode=disable")
	t.Setenv(ppRateLimitTenantEnv, "tenant-123")

	l := NewAdaptiveLimiterForHost(8.0, "api.example.com")
	if _, ok := l.backend.(*postgresRateLimitBackend); !ok {
		t.Fatalf("backend = %T, want *postgresRateLimitBackend", l.backend)
	}
	if l.key != (rateLimitKey{Host: "api.example.com", TenantID: "tenant-123"}) {
		t.Fatalf("key = %#v, want host+tenant key", l.key)
	}
}

func TestPostgresRateLimitBackendUsesSelectForUpdate(t *testing.T) {
	if !strings.Contains(selectRateLimitStateForUpdateSQL, "FOR UPDATE") {
		t.Fatalf("Postgres rate-limit state query must lock row with SELECT FOR UPDATE: %s", selectRateLimitStateForUpdateSQL)
	}
	if !strings.Contains(insertRateLimitStateSQL, "ON CONFLICT (host, tenant_id) DO NOTHING") {
		t.Fatalf("Postgres rate-limit insert must key row by host and tenant_id: %s", insertRateLimitStateSQL)
	}
}

func TestAdaptiveLimiter_SharedBackendSerializesWaitSchedule(t *testing.T) {
	backend := newMemoryRateLimitBackend()
	l1 := NewAdaptiveLimiterForHost(10.0, "https://api.example.com")
	l2 := NewAdaptiveLimiterForHost(10.0, "https://api.example.com")
	l1.backend = backend
	l2.backend = backend
	...
	if elapsed := time.Since(start); elapsed < 80*time.Millisecond {
		t.Fatalf("shared backend second Wait() took %v, want >= 80ms", elapsed)
	}
}
...
func rateLimitResponse(limit, remaining, window string) *http.Response {
	resp := &http.Response{Header: http.Header{}}
	resp.Header.Set("X-RateLimit-Limit", limit)
	resp.Header.Set("X-RateLimit-Remaining", remaining)
	resp.Header.Set("X-RateLimit-Window", window)
	resp.Header.Set("X-RateLimit-Context", "test")
	return resp
}
