...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  follow-up-boss-pp-cli doctor
  follow-up-boss-pp-cli doctor --json
  follow-up-boss-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export FUB_API_KEY=<your-key>"
					report["auth_key_url"] = "https://docs.followupboss.com/reference/authentication"
					report["auth_instructions"] = "Use a Follow Up Boss API key; the printed CLI encodes it as Basic key-colon auth."
				} else {
					authConfigured = true
			...
			if os.Getenv("FUB_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "FUB_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "FUB_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'follow-up-boss-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'follow-up-boss-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'follow-up-boss-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'follow-up-boss-pp-cli sync' to refresh."
	}
	return report
}
