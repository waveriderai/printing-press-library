...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  follow-up-boss-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  follow-up-boss-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"action-plans", "action-plans-people", "appointment-outcomes", "appointment-types", "appointments", "automations", "automations-people", "calls", "custom-fields", "deal-custom-fields", "deals", "em-campaigns", "em-events", "events", "groups", "groups-round-robin", "identity", "me", "people", "people-check-duplicate", "people-relationships", "people-unclaimed", "pipelines", "ponds", "smart-lists", "stages", "tasks", "team-inboxes", "teams", "templates", "text-message-templates", "text-messages", "timeframes", "users", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  follow-up-boss-pp-cli workflow status

  # Show status as JSON
  follow-up-boss-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("follow-up-boss-pp-cli")
			}
