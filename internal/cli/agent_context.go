...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	envVars := []agentContextAuthEnvVar{
		...
		// PATCH: Follow Up Boss also supports optional partner integration
		// headers and OAuth helper credentials.
		{
			Name:        "FUB_SYSTEM",
			Kind:        "per_call",
			Required:    false,
			Sensitive:   false,
			Description: "Optional Follow Up Boss integration system name sent as X-System.",
		},
		{
			Name:        "FUB_SYSTEM_KEY",
			Kind:        "per_call",
			Required:    false,
			Sensitive:   true,
			Description: "Optional Follow Up Boss integration system key sent as X-System-Key.",
		},
		{
			Name:        "FUB_CLIENT_ID",
			Kind:        "oauth",
			Required:    false,
			Sensitive:   false,
			Description: "OAuth client ID for auth oauth-url/oauth-token/oauth-refresh.",
		},
		{
			Name:        "FUB_CLIENT_SECRET",
			Kind:        "oauth",
			Required:    false,
			Sensitive:   true,
			Description: "OAuth client secret for token exchange and refresh.",
		},
	}
	authMode := "api_key_basic_or_oauth"
	if authMode == "" {
		authMode = "none"
	}
