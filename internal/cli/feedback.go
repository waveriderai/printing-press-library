...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".follow-up-boss-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FOLLOW_UP_BOSS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FOLLOW_UP_BOSS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "follow-up-boss-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.follow-up-boss-pp-cli/feedback.jsonl.
When ` + "`FOLLOW_UP_BOSS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FOLLOW_UP_BOSS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  follow-up-boss-pp-cli feedback list
  follow-up-boss-pp-cli feedback list --limit 5
  follow-up-boss-pp-cli feedback list --json`,
