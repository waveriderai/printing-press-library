# Benzinga Printed CLI Agent Guide

This directory is a generated `benzinga-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
benzinga-pp-cli doctor --json
benzinga-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
benzinga-pp-cli which "<capability>" --json
benzinga-pp-cli <command> --help
```
...
```bash
benzinga-pp-cli <command> --help
benzinga-pp-cli <command> --dry-run --agent
```
