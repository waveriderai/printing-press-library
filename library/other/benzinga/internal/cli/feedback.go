...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BENZINGA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BENZINGA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "benzinga-pp-cli/feedback")
...
When ` + "`BENZINGA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BENZINGA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  benzinga-pp-cli feedback list
  benzinga-pp-cli feedback list --limit 5
  benzinga-pp-cli feedback list --json`,
