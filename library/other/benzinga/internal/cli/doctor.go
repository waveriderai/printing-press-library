...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  benzinga-pp-cli doctor
  benzinga-pp-cli doctor --json
  benzinga-pp-cli doctor --fail-on warn
  benzinga-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set one of: export BENZINGA_API_KEY=\"your-token-here\" or export CALENDAR_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "BENZINGA_API_KEY")
				if os.Getenv("BENZINGA_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "BENZINGA_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("BENZINGA_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "BENZINGA_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "BENZINGA_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CALENDAR_API_KEY")
				if os.Getenv("CALENDAR_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "CALENDAR_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CALENDAR_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CALENDAR_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "CALENDAR_API_KEY optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "benzinga-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'benzinga-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'benzinga-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'benzinga-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'benzinga-pp-cli sync' to refresh."
	}
	return report
}
