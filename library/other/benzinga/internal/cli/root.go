...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "benzinga-pp-cli",
		Short: `Benzinga CLI — Every Benzinga calendar, news, fundamentals, and signal endpoint as a typed command — plus an offline SQLite store, ful…`,
		Long: `Benzinga CLI — Every Benzinga calendar, news, fundamentals, and signal endpoint as a typed command — plus an offline SQLite store, ful…
...
Health check: run 'benzinga-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("benzinga-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'benzinga-pp-cli profile list')")
	...
	rootCmd.AddCommand(newGovCmd(flags))
	rootCmd.AddCommand(newLogosCmd(flags))
	rootCmd.AddCommand(newNewsCmd(flags))
	rootCmd.AddCommand(newQuoteDelayedCmd(flags))
	rootCmd.AddCommand(newSecCmd(flags))
	rootCmd.AddCommand(newSignalCmd(flags))
	rootCmd.AddCommand(newTrendingTickersCmd(flags))
	...
	rootCmd.AddCommand(newNovelAnalystAccuracyCmd(flags))
	rootCmd.AddCommand(newNovelCatalystsCmd(flags))
	rootCmd.AddCommand(newNovelEarningsSeasonCmd(flags))
	rootCmd.AddCommand(newNovelInsiderClusterCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newNovelWhyCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAnalystPromotedCmd(flags))
	rootCmd.AddCommand(newBarsPromotedCmd(flags))
	rootCmd.AddCommand(newBullsBearsSayPromotedCmd(flags))
	rootCmd.AddCommand(newCalendarRemovedPromotedCmd(flags))
