...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  benzinga-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  benzinga-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"analyst", "calendar", "calendar-conference-calls", "calendar-dividends", "calendar-earnings", "calendar-economics", "calendar-fda", "calendar-guidance", "calendar-ipos", "calendar-offerings", "calendar-ratings", "calendar-ratings-analysts", "calendar-ratings-firms", "calendar-removed", "calendar-splits", "consensus-ratings", "erx-gaps", "gov", "gov-usa-congress-trades-reports", "market", "news", "news-channels", "news-removed", "sec", "signal", "signal-halt-resume", "signal-option-activity"}
			// Bound per-resource pages so a full archive completes inside the
			// live-dogfood per-command timeout; real runs page fully.
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  benzinga-pp-cli workflow status

  # Show status as JSON
  benzinga-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("benzinga-pp-cli")
			}
