...
func printOutputWithFlags(w io.Writer, data json.RawMessage, flags *rootFlags) error {
	...
	// --plain: render as plain tab-separated text (one record per line, no
	// table borders or alignment) for piping into cut/awk and other line tools.
	if flags.plain {
...
// printPlain renders array data as tab-separated values with a header row.
// Unlike printCSV it uses tabs (not commas) and does not quote fields, so the
// output is trivially splittable with cut -f / awk -F'\t'. Non-array payloads
// fall back to their raw JSON form.
	...
	for _, item := range items {
		...
		for _, k := range keys {
			if v := item[k]; v == nil {
				...
				// Collapse embedded tabs/newlines so each record stays on one line.
				s := fmt.Sprintf("%v", v)
