...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "autotempest-pp-cli",
		Short: `AutoTempest CLI — Every AutoTempest car-search source in your terminal, with a local store, cross-source VIN dedupe, and price-drop track…`,
		Long: `AutoTempest CLI — Every AutoTempest car-search source in your terminal, with a local store, cross-source VIN dedupe, and price-drop track…
...
Health check: run 'autotempest-pp-cli doctor' to verify connectivity (no auth required).
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("autotempest-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'autotempest-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAuctionsCmd(flags))
	rootCmd.AddCommand(newNovelDealCmd(flags))
	rootCmd.AddCommand(newNovelDedupeCmd(flags))
	rootCmd.AddCommand(newNovelDropsCmd(flags))
	rootCmd.AddCommand(newNovelSpreadCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newMakesPromotedCmd(flags))
