...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "autotempest-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AUTOTEMPEST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AUTOTEMPEST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "autotempest-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/autotempest-pp-cli/feedback.jsonl.
When ` + "`AUTOTEMPEST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AUTOTEMPEST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  autotempest-pp-cli feedback list
  autotempest-pp-cli feedback list --limit 5
  autotempest-pp-cli feedback list --json`,
