...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "shipsgo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHIPSGO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHIPSGO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "shipsgo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/shipsgo-pp-cli/feedback.jsonl.
When ` + "`SHIPSGO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHIPSGO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shipsgo-pp-cli feedback list
  shipsgo-pp-cli feedback list --limit 5
  shipsgo-pp-cli feedback list --json`,
