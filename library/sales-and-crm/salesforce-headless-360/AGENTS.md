# Salesforce Headless 360 Printed CLI Agent Guide

This directory is a generated `salesforce-headless-360-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
salesforce-headless-360-pp-cli doctor --json
...
```bash
salesforce-headless-360-pp-cli which "<capability>" --json
salesforce-headless-360-pp-cli <command> --help
```
...
```bash
salesforce-headless-360-pp-cli <command> --agent
```
...
```bash
salesforce-headless-360-pp-cli <command> --help
salesforce-headless-360-pp-cli <command> --dry-run --agent
```

Use `--yes --no-input` only after the target, arguments, and side effects are clear. Write commands (`agent update`, `agent upsert`, `agent advance`, `agent close-case`, `agent note`, `agent log-activity`, plan execution) audit locally and support idempotency keys; prefer passing one for any retryable mutation.
