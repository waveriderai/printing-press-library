# Follow Up Boss Printed CLI Agent Guide

This directory is a generated `follow-up-boss-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
follow-up-boss-pp-cli doctor --json
follow-up-boss-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
follow-up-boss-pp-cli which "<capability>" --json
follow-up-boss-pp-cli <command> --help
```
...
```bash
follow-up-boss-pp-cli <command> --agent
```
...
```bash
follow-up-boss-pp-cli <command> --help
follow-up-boss-pp-cli <command> --dry-run --agent
```
